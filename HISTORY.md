...
### New Features
- Behavioral particles for the 1D network solver (PR 1 of the design in `docs/superpowers/specs/2026-09-16-network-behavioral-particles-design.md`): `NetworkSolver` is the base class and the passive model, with `on_release` and `behave` hooks, a per-particle velocity factor in advection, declared per-particle state (`StateVar`) that the writer and `NetworkResults` handle generically, a model registry (`PARTICLE_MODELS`, `[network.particles] model = "passive" | "drift" | "package.module:Class"`), and the terminal statuses `settled` (3) and `removed` (4) with `NetworkResults.terminal(status)`. The `drift` model resolves a relative elevation per particle: log-law velocity profile, vertical mixing by a kernel that keeps a uniform column exactly uniform (a random rotation on the sphere for the parabolic profile, a reflected Gaussian step for a constant one), settling or swimming with an optional diel cycle, and deposition as a Robin bed condition with a deposition velocity and Krone's critical-shear suppression. `[network.dispersion]` gains `background`, `shear_correction` (the resolved vertical shear is removed from the Fischer coefficient by Taylor's integral on the walk's own domain) and the `vertical` sub-table (`profile`, `kappa`, `beta`, `value`, `background`, `scale`, `velocity_profile`), mirroring the 2D/3D `lev + beta u* h` parameterization. The passive model is unchanged and bit-identical for the same seed. Analytical acceptance tests: uniform column stays uniform, Rouse profile, mean advection, Taylor shear dispersion with and without the Fischer correction, Robin deposition against a finite-volume reference. `scipy` is now a runtime dependency; the interval reflection of `Particles.validate_z` moved to `fluvial_particle.random_walk.reflect_interval`, shared by both solvers

### Bug Fixes
- `RiverGrid` sets a zero `vtkProbeFilter` tolerance so cell location stays exact on VTK 9.7 (#41). `vtkProbeFilter` hands its auto-computed tolerance (0.1% of the largest cell diagonal) to the cell locator; VTK 9.7's locators honour it where earlier ones ignored it, so points within a few millimetres of a cell face were assigned to the neighbouring cell: wet/dry checks flipped at the bank and velocities near cell faces were extrapolated from the neighbouring cell, which is why every 2D/3D regression case failed under 9.7 against the unchanged fixtures. The deprecated find-cell strategy, still required for exactness below 9.7, is only built and attached there. Points exactly on a node line are valid on every version but may land in either adjacent cell depending on the version; only off-line points are version-stable. `tests/test_probe_tolerance.py` guards the 2D bank, the 3D probe on the curvilinear meander grid, exact-boundary validity, and the MPI probe class
...
### Infrastructure
- Lifted the `vtk<9.7` pin added in 0.1.0 (#41); CI runs the test suite on both VTK 9.6 and 9.7 (`nox -s "tests-3.13(vtk96)"`, `"tests-3.13(vtk97)"`); `environment-mpi.yml` aligned to `vtk>=9.3.1`
