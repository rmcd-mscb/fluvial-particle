...
``[network.particles]`` names the particle model; absent, the run is the passive model and is
bit-identical to a run without the table for the same seed.
...
0.59 m, ``u*`` 0.10 m/s) the default is about 470 sub-steps per 900 s step, roughly 4 s of vectorized
...
startup report prints the count and the reaches that hit the cap.
...
.. list-table::
   ...
   * - ``critical_ustar``
     - none
     - m/s; no deposition where the reach ``u*`` exceeds it
   * - ``zeta_min``
     - 0.001
...
what vertical shear produces (Elder's term, ``5.93 h u*`` for the log law with parabolic mixing,
Elder 1959) and what transverse shear produces, the larger part in a wide channel; Fischer's formula
(Fischer et al. 1979) is an empirical fit to the total and contains both. A model that resolves the
vertical generates the vertical-shear part from its own kinematics, so the kick must supply only the
transverse remainder or the vertical part is counted twice: 3 percent of Fischer at the Delaware
medians, 60 percent in a 5 m wide, 1 m deep, 0.3 m/s stream. With ``shear_correction = "auto"`` the
correction is applied whenever the model resolves the vertical and the velocity profile is not
uniform, and never otherwise; ``"on"`` and ``"off"`` override, and ``"on"`` with a model that does
not resolve the vertical is a configuration error. The corrected coefficient is
...
the walk applies it. On the full column with the unclipped log law ``c`` is Elder's
...
Delaware median ratio 0.14). ``c`` depends on the reach only through ``u* / v`` and is tabulated once
at start-up. The acceptance tests show the resolved shear reproduces ``2 c u* h t`` within 10
percent and that, with Fischer on and the correction active, the total variance is ``2 K t``.
...
``k_d dt_sub`` exceeds ``zeta_min h`` the probability clips at 1 and the rate depends on the sub-step
count; the startup report prints the probability and flags the clip (raise ``zeta_min`` or lower
``substep_fraction``). A perfectly absorbing bed carries the ``sqrt(dt)`` bias of absorbing random
walks. Rouse numbers ``w / (kappa u*)`` of 1 and above concentrate particles in a bed layer thinner
than any affordable sub-step resolves; the model then reproduces the near-bed profile only
qualitatively. When the sub-step count hits ``max_substeps`` the vertical walk resolves less of the
shear dispersion than the fraction implies.
