...
class Particle:
    ...
    def __init__(self, index, x, y, z, time_offset, amplitude, period, min_elev):
        ...
        self.y = y
        self.z = z
        #         self.px = array.array('d', [x])
        #         self.py = array.array('d', [y])
        #         self.pz = array.array('d', [z])
        #         self.time = array.array('d', [0.0])
        #         self.bedElev = array.array('d', [0])
        #        self.wse = array.array('d', [0])
        self.lastx = x
        self.lasty = y
        self.lastz = z
        self.time = 0.0
    ...
    def move(self, index, vx, vy, vz, dx, dy, xrnum, yrnum, dt):
        """Update position based on speed, angle."""
        #         self.x = self.px[index-1] + (vx*dt) + xrnum*math.sqrt(2.0*dx*dt)
        #         self.y = self.py[index-1] + (vy*dt) + yrnum*math.sqrt(2.0*dy*dt)
        #         self.z = self.pz[index-1] + (vz*dt)
        velmag = math.sqrt((vx * vx) + (vy * vy))
        dl = xrnum * math.sqrt(2.0 * dx * dt)
        dt = yrnum * math.sqrt(2.0 * dy * dt)
        if velmag == 0:
            tmpx = self.x + (vx * dt)
            tmpy = self.y + (vy * dt)
        else:
        ...
        #         tmpx = self.lastx + (vx*dt) + xrnum*math.sqrt(2.0*dx*dt)
        #         tmpy = self.lasty + (vy*dt) + yrnum*math.sqrt(2.0*dy*dt)
        tmpz = self.z + (vz * dt)
        return tmpx, tmpy, tmpz

    def movewithdrift(
        self, index, vx, vy, vz, dx, dy, dddx, dddy, xrnum, yrnum, dt, type=1
    ):
        """Update position based on speed, angle."""
        #         self.x = self.px[index-1] + (vx*dt) + xrnum*math.sqrt(2.0*dx*dt)
        #         self.y = self.py[index-1] + (vy*dt) + yrnum*math.sqrt(2.0*dy*dt)
        #         self.z = self.pz[index-1] + (vz*dt)
        velmag = math.sqrt((vx * vx) + (vy * vy))
        dgradmag = math.sqrt((dddx * dddx) + (dddy * dddy))
        dl = xrnum * math.sqrt(2.0 * dx * dt)
        dt = yrnum * math.sqrt(2.0 * dy * dt)
        if velmag == 0:
            tmpx = self.x + ((vx + dddx) * dt)
            tmpy = self.y + ((vy + dddy) * dt)
        else:
            if type == 1:
                tmpx = (
                    self.x
                    + (vx * dt)
                    + dddx * dt
                    + (dl * vx / velmag - dt * vy / velmag)
                )
                tmpy = (
                    self.y
                    + (vy * dt)
                    + dddy * dt
                    + (dl * vy / velmag + dt * vx / velmag)
                )
            else:
                tmpx = (
                    self.x
                    + ((vx) * dt)
                    + (dddx / dgradmag - dddy / dgradmag) * dt
                    + (dl * vx / velmag - dt * vy / velmag)
                )
                tmpy = (
                    self.y
                    + ((vy) * dt)
                    + (dddx / dgradmag + dgradmag / dgradmag) * dt
                    + (dl * vy / velmag + dt * vx / velmag)
                )
    ...
    def move3d(self, index, vx, vy, vz, dx, dy, dz, xrnum, yrnum, zrnum, dt):
        """Update position based on speed, angle."""
        velmag = math.sqrt((vx * vx) + (vy * vy))
        dl = xrnum * math.sqrt(2.0 * dx * dt)
        dt = yrnum * math.sqrt(2.0 * dy * dt)
        dv = zrnum * math.sqrt(2.0 * dz * dt)
        if velmag == 0:
            tmpx = self.x + (vx * dt)
            tmpy = self.y + (vy * dt)
            tmpz = self.z + (vz * dt)
    ...
    def vert_random_walk(self, time, bedelev, wse, vx, vy, vz, dz, zrnum, dt):
    ...
    def move_random_only_2d(self, index, dx, dy, xrnum, yrnum, dt):
        """Update position based on speed, angle."""
        #         self.x = self.px[index-1] + xrnum*math.sqrt(2.0*dx*dt)
        #         self.y = self.py[index-1] + yrnum*math.sqrt(2.0*dy*dt)
        #         self.z = self.pz[index-1]
        tmpx = self.x + xrnum * math.sqrt(2.0 * dx * dt)
        tmpy = self.y + yrnum * math.sqrt(2.0 * dy * dt)
        tmpz = self.z
        #         print 'in random dx: %f dy: %f' %(xrnum*math.sqrt(2.0*dx*dt),yrnum*math.sqrt(2.0*dy*dt))
        return tmpx, tmpy, tmpz

    def move_random_only_3d(self, index, dx, dy, dz, xrnum, yrnum, zrnum, dt):
        """Update position based on speed, angle."""
        tmpx = self.x + xrnum * math.sqrt(2.0 * dx * dt)
        tmpy = self.y + yrnum * math.sqrt(2.0 * dy * dt)
        tmpz = self.z + zrnum * math.sqrt(2.0 * dz * dt)
        #         print 'in random dx: %f dy: %f' %(xrnum*math.sqrt(2.0*dx*dt),yrnum*math.sqrt(2.0*dy*dt))
        return tmpx, tmpy, tmpz
